...
namespace DientesLimpios.Pruebas.Dominio.Entidades
{
    ...
    public class CitaTests
    {
        ...
        [TestMethod]
        public void Contructor_CitaValida_EstadoProgramada()
        {
        ...
        [TestMethod]
        public void Contructor_FechaInicioPosteriorFechaActual_lanzaExcepcion()
        {
            var intervaloInvalido = new IntervaloDeTiempo(DateTime.UtcNow.AddDays(-1), DateTime.UtcNow);
            Assert.ThrowsExactly<ExcepcionDeReglaDeNegocio>(() => new Cita(_pacienteId, _dentistaId, _consultorioId, intervaloInvalido));
        }

        [TestMethod]
        public void Cancelar_CitaProgramada_CambioEstadoCancelada()
        {
            var cita = new Cita(_pacienteId, _dentistaId, _consultorioId, _intervalo);
            cita.Cancelar();
        ...
        [TestMethod]
        public void Completar_CitaProgramada_CambioEstadoCompletada()
        {
            var cita = new Cita(_pacienteId, _dentistaId, _consultorioId, _intervalo);
            cita.Completar();
